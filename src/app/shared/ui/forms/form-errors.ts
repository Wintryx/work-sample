...
const findValidatorMessage = (
  validators: FieldValidatorConfig[],
  type: FieldValidatorConfig["type"],
): string | undefined => validators.find((v) => v.type === type)?.message;
...
export const resolveFieldErrorMessage = (
  ...
  if (control.hasError("min")) {
    ...
    return (
      message?.replace("{value}", String(error?.min ?? "")) ??
  ...
  if (control.hasError("max")) {
    ...
    return (
      message?.replace("{value}", String(error?.max ?? "")) ??
  ...
  if (control.hasError("minlength")) {
    ...
    const message = findValidatorMessage(validators, "min");
    return (
      message?.replace("{value}", String(error?.requiredLength ?? "")) ??
